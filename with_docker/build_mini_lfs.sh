#!/usr/bin/bash
#-------------------
[ -z "$LFS" ] && echo "[ERROR]: environment variable LFS not set" && exit 1
[ -z "$LFS_TGT" ] && echo "[ERROR]: environment variable LFS_TGT not set" && exit 2    
...
{
    [ ! -d "$LFS" ] && echo "[ERROR]: Directory $LFS does not exist yet." && exit 3
    #create build directories
    mkdir -v $LFS/{sources,tools}    
    chmod -v a+wt $SRC
    ln -svf $LFS/tools /
    wget -c http://www.linuxfromscratch.org/lfs/view/stable/wget-list -o $LFS/wget-list
...
function archive_artefact ()
{
    ...
    [ ! -d "$LFS/results" ] && mkdir -v $LFS/results
    tar cvf $LFS/results/tools.tar $LFS/tools
    gzip -v $LFS/results/tools.tar
    tar cvf $LFS/results/sources.tar $SRC
