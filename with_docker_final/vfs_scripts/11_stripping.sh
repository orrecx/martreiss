...
function stripp () 
{
	$LFS/tools/bin/find /usr/lib -type f -name \*.a \
	   -exec /tools/bin/strip --strip-debug {} ';'

	$LFS/tools/bin/find /lib /usr/lib -type f \( -name \*.so* -a ! -name \*dbg \) \
	   -exec /tools/bin/strip --strip-unneeded {} ';'

	$LFS/tools/bin/find /{bin,sbin} /usr/{bin,sbin,libexec} -type f \
